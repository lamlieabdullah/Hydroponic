...
DFRobot_EC::DFRobot_EC()
{
    ...
    this->_ecCalibrationFinish    = 0;
    this->_enterCalibrationFlag   = 0;
    this->callStatus = "";
...
void DFRobot_EC::ecCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        case 2:
        if(enterCalibrationFlag){
            if((this->_rawEC>0.9)&&(this->_rawEC<2.5)){                         //recognize 1.413us/cm buffer solution
                compECsolution = 1.413*(1.0+0.0185*(this->_temperature-25.0));  //temperature compensation
                callStatus =  "Done. Buff 1413"; //Oee
                ...
                callStatus =  "Done. Buff 12.88"; //Oee
            }else{
                Serial.print(F(">>>Buffer Solution Error Try Again<<<   "));
                callStatus =  "Buf Err! Retry!"; //Oee
                ecCalibrationFinish = 0;
            }
            ...
            if((KValueTemp>0.5) && (KValueTemp<1.5)){
                ...
                callStatus =  ">>>Failed"; //Oee
                Serial.println();
            //    strncpy( callStatus, "Failed,Try Again", sizeof(callStatus) ); //Oee
        ...
        if(enterCalibrationFlag){
                ...
                if(ecCalibrationFinish){   
                    ...
                    //EEPROM.commit(); Kena click save.
                    Serial.print(F(">>>EEPROM committed, Calibration Successful"));
            //        strncpy( callStatus, "EEPROM committed", sizeof(callStatus) ); //Oee
                }else{
                    Serial.print(F(">>>Calibration Failed"));
            //        strncpy( callStatus, "Calibration Failed", sizeof(callStatus) ); //Oee
                }
