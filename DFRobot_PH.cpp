...
void DFRobot_PH::phCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        case 2:
        if(enterCalibrationFlag){
            if((this->_voltage>1322)&&(this->_voltage<1700)){        // buffer solution:7.0{
                Serial.println();
                Serial.print(F(">>>Buffer Solution:7.0"));
                callStatus =  "Done. ph 7.0"; //Oee
                ...
                Serial.println(); 
                callStatus =  "Buf Err! Retry!"; //Oee                                   // not buffer solution or faulty operation
                phCalibrationFinish = 0;
            }
        }
        break;
